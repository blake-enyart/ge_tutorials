...
def load_files_into_db(ds, **kwargs):
    ...
    with engine.connect() as conn:
        ...
        df_npi_small = pd.read_csv(
            os.path.join(GE_TUTORIAL_ROOT_PATH, "data", "npi_small.csv")
        )
        column_rename_dict = {
        ...
        files = ["state_abbreviations", "contacts"]
...
def validate_source_data_load(ds, **kwargs):
    ...
    # batch_kwargs_db = {'query': 'SELECT YOUR_ROWS FROM YOUR_TABLE', 'datasource': datasource_name_file_db}
    ...
    run_id = (
        "airflow:" + kwargs["dag_run"].run_id + ":" + str(kwargs["dag_run"].start_date)
    )
    results = context.run_validation_operator(
        "action_list_operator", assets_to_validate=[batch_file, batch_db], run_id=run_id
...
def validate_analytical_output(ds, **kwargs):
    ...
    datasource_name = (
        "datawarehouse"  # a datasource configured in your great_expectations.yml
    )
    ...
    expectation_suite_name = (
        "count_providers_by_state.critical"  # this is an example of
    )
...
# DAG dependencies
task_validate_source_data >> task_load_files_into_db >> task_validate_source_data_load >> task_transform_data_in_db >> task_transform_data_to_mart >> task_validate_analytical_output
# task_publish
